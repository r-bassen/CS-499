...
export class AuthenticationService {
  ...
  // FIX ME: Define the key we will use to store our JWT in Storage
  constructor(
  ...
  public getToken(): string {
    let out: any;
    out = localStorage.getItem('token');   // FIX ME: Retrieve the token from Storage
  ...
  public saveToken(token: string): void {
    localStorage.setItem('token', token); // FIX ME: Retrieve the token from Storage
  }

  // Logout of our application and remove the JWT from Storage 
  public logout(): void {
    localStorage.removeItem('token'); // FIX ME: Retrieve the token from Storage
  }
  ...
  public login(user: User, passwd: string) {
    // FIX ME: Uncomment below to enable subscription handling here
